...
Admin → Ticket Workspace now controls the organization default. Every technician can open Customize Layout on a ticket to drag/reorder widgets across columns, use accessible move controls, hide optional widgets, save a personal server-side override, or return to the live organization default. Layout JSON is versioned and defensively parsed: every known widget must occur exactly once, unknown/duplicate widgets are rejected, and core workflow widgets cannot be hidden. Migration `0009_greedy_spyke.sql` adds nullable organization and user layout preferences; null user preference deliberately means “follow the organization,” not a copied snapshot.

Ticket details also have their own authenticated `(workspace)` route layout. Ticket-list and dashboard rows open that URL in a separate browser workspace window; the full application sidebar is omitted in favor of a compact ticket header with Back to Tickets and Close controls. The direct URL remains session-gated and usable on its own.

Reduced form noise after the first workspace review: Customer/Contract, Classification, and Ticket Details are read-only widgets by default, with one page-level Edit Ticket panel and one Save Ticket action for all structural fields. Assignment remains continuously available as a low-risk operational picker and autosaves on selection. Record-creation and workflow actions (notes, time, triage, status transitions) retain explicit buttons. Ticket-number links now use the same popup launcher as ticket titles/rows instead of opening an inconsistent ordinary tab.

The follow-up interaction pass simplified record creation too. Notes now use one body field plus a checked-by-default Internal note checkbox; unchecking it marks the note client-visible. Log Time is no longer a permanently expanded form: its rail widget shows accumulated ticket minutes and opens a focused Add Time Entry modal for date, duration, work performed, and billable state.

Expanded that modal after reviewing Autotask's 2026.3 time-entry workflow. The 920px sectioned dialog now captures organization-local work date, start and end time, explicit overnight work, calculated duration, inherited contract/billing context, required customer-facing summary notes, optional internal notes, and an optional valid ticket-status transition. It offers Save & New and Save & Close. Migration `0010_blue_ultragirl.sql` adds nullable UTC `start_at`/`end_at` instants and `internal_notes`; historical entries remain valid. Work Type, Role, Service/Bundle, billing offset, and show-on-invoice remain deliberately deferred until Keep has the supporting admin and billing models.

A second Autotask-informed pass moved record creation to the Activity header: New Note and New Time Entry now sit together, with Attachments deliberately deferred until storage and permissions are designed. The former Log Time rail widget is now a read-only Time Summary showing worked, estimated, and remaining minutes; estimated work is editable with the ticket. The modal's Time Entry Details now includes six selectable days, a calendar fallback, existing-entry blocks, and a draggable full-day timeline that updates start/end in 15-minute increments. Billing offset supports add/subtract minutes and calculates time to bill. Migration `0011_wide_speed_demon.sql` adds `tickets.estimated_minutes` and `time_entries.billing_offset_minutes`.
